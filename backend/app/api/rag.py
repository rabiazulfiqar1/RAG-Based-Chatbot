"""
RAG API Router - Clean API endpoints using modular services.
"""
...
async def upload_document(
    ...
    current_count = get_session_document_count(session_id, user_id)
    ...
    try:
        if file:
            ...
            try:
                # Process document
                docs = load_document_by_type(tmp_path, filename)
                chunk_count = chunk_and_embed_document(docs, session_id, document_id, filename, user_id)
...
async def enhanced_chat(
    ...
    Semantic caching:
    - Checks semantic cache FIRST (session-independent)
    ...
    try:
        ...
        if not selected_document_ids:
            ...
            if cached_response:
                ...
                save_messages_to_db(session_id, message, cached_response, source="semantic_cache")
        ...
        if document_answer and "cannot find" not in document_answer.lower():
            ...
            save_messages_to_db(session_id, message, full_response, source=source)
            ...
            ensure_session_exists(request.session_id, user_id)
            final_history = get_conversation_context(request.session_id, K=5, SUMMARY_BATCH_SIZE=5)
            ...
            async def save_after_stream():
                ...
                save_messages_to_db(session_id, message, complete_response, source="general")
...
async def get_messages(session_id: str, limit: int = 50, offset: int = 0):
    ...
    messages = get_user_messages(session_id, limit, offset)
    return messages or []